...
vec3 fractalNoise(vec3 pos, float time, float luminance) {
    float noise = 0.0;
    float amplitude = 1.0;
    float frequency = 1.0;

    // Luminance modulates initial frequency - prevents uniform bands
    float lumFreqScale = 0.7 + luminance * 0.6;

    for(int i = 0; i < 4; i++) {
        // Add luminance-based phase offset to break patterns
        float lumPhaseOffset = luminance * float(i) * 2.0;
        noise += sin(dot(pos * frequency * lumFreqScale, vec3(12.9898, 78.233, 45.164)) + time + lumPhaseOffset) * amplitude;
        amplitude *= 0.5;
        frequency *= 2.0;
    }
...
vec2 textureFluctuation(vec2 uv, float time, float depth) {
    // Distance-based frequency scaling - far away gets lower frequency
    float distanceScale = 1.0 / max(depth * 0.5, 1.0);

    // Use 10x scaled coordinates for wave calculations to spread out effects
    vec2 effectUV = uv * 10.0 * distanceScale;

    float fluctuation = sin(time * 1.3 + effectUV.x * 8.0) * 0.02;
    fluctuation += cos(time * 0.9 + effectUV.y * 6.0) * 0.015;
    fluctuation += sin(time * 2.1 + depth * 3.0) * 0.01;

    // Add depth-based breathing effect
    float breathing = sin(time * 0.7 + depth * 2.0) * 0.005;

    // Scale amplitude by distance too
    return uv + vec2(fluctuation, breathing) * distanceScale;
}
...
vec2 luminanceFluctuation(vec2 uv, float time, float depth, float luminance) {
    ...
    // Use 10x scaled luminance for wave calculations
    float effectLum = luminance * 10.0;
    ...
    float luminanceBreathing = sin(time * 1.1 + effectLum * 4.0) * 0.003 * luminanceMod;
...
vec3 specularOverlay(vec3 color, vec2 uv, float time, float depth) {
    ...
    // Create displacement-like visual effects - REDUCED FREQUENCIES
    vec3 displacementEffect = vec3(
        sin(specData.r * 2.0 + time * 2.0) * 0.05,
        cos(specData.g * 1.5 + time * 1.5) * 0.05,
        sin(specData.b * 2.5 + time * 2.5) * 0.05
    );
...
vec3 depthShadows(vec3 color, float depth, float time) {
    // Calculate luminance to modulate shadow oscillations
    float lum = calculateLuminance(color);
    float lumPhase = lum * 4.0;

    // Create shadow based on depth and time
    float shadowIntensity = 1.0 - pow(depth * 0.3, 1.5);

    // Add time-based shadow movement - luminance modulates frequency
    float shadowPhase = sin(time * 0.8 * (0.8 + lum * 0.4) + depth * 4.0 + lumPhase) * 0.3 + 0.7;
    shadowIntensity *= shadowPhase;

    // Create depth-based color shift - luminance affects shift amount
    float depthShift = sin(depth * 2.0 * (0.6 + lum * 0.8) + time * 1.2 + lumPhase * 0.5) * 0.1;
...
vec3 depthDynamics(vec3 color, float depth, float time) {
    // Calculate luminance to modulate frequency
    float lum = calculateLuminance(color);
    float lumPhase = lum * 3.14159;

    // Depth-based color temperature - luminance modulates frequency
    float temperature = sin(depth * 3.0 * (0.5 + lum * 1.5) + time * 0.6 + lumPhase) * 0.2 + 0.8;
    color.r *= temperature;
    color.b *= 2.0 - temperature;

    // Depth-based saturation - luminance affects oscillation
    float saturation = cos(depth * 2.5 * (0.7 + lum * 0.6) + time * 1.1 + lumPhase * 0.5) * 0.3 + 0.7;
    float luminance = dot(color, vec3(0.299, 0.587, 0.114));
    color = mix(vec3(luminance), color, saturation);

    // Depth-based contrast - bright areas get more contrast variation
    float contrast = sin(depth * 4.0 * (0.8 + lum * 0.4) + time * 0.9 + lumPhase * 1.2) * 0.2 + 1.0;
...
subroutine (spell) vec4 emptySpell(vec4 color) {
    ...
    float lum = calculateLuminance(color.rgb);

    // Apply basic psychedelic effects even in empty spell
    vec3 pos = model_Coords.xyz;
    vec3 noise = fractalNoise(pos, u_time, lum);
...
subroutine (spell) vec4 castMining(vec4 color) {
    ...
    float resultLum = calculateLuminance(result.rgb);
    vec3 noise = fractalNoise(model_Coords.xyz, u_time * 3.0, resultLum);
...
subroutine (spell) vec4 releaseMining(vec4 color) {
    ...
    // Fractal explosion
    float releaseLum = calculateLuminance(result.rgb);
    vec3 fractal = fractalNoise(model_Coords.xyz * 2.0, u_time * 5.0, releaseLum);
...
void main(){
    ...
    // UV distortion for warping effect - ONLY in dark areas, reduced frequency
    vec2 distortedUV = texture_Coords.xy;

    // Distance-based frequency scaling for LOD
    float distanceScale = 1.0 / max(zDepth * 0.5, 1.0);

    // Use 10x scaled coordinates for effect calculations, scaled by distance
    vec2 effectDistortedUV = distortedUV * 10.0 * distanceScale;

    // Only apply to dark areas - bright areas stay clean
    float darkAreaMask = pow(1.0 - luminance, 2.0); // Strong falloff in bright areas

    // Reduced frequencies to avoid banding
    float luminancePhase = luminance * 6.28318; // 0-2π based on brightness

    distortedUV += sin(u_time * 2.0 + effectDistortedUV.x * 2.0 + luminancePhase) * 0.08 * adaptiveIntensity * darkAreaMask * distanceScale;
    distortedUV += cos(u_time * 1.5 + effectDistortedUV.y * 1.5 + luminancePhase * 0.7) * 0.08 * adaptiveIntensity * darkAreaMask * distanceScale;
    ...
    // Add fractal noise overlay with adaptive intensity - scale down at distance
    float noiseDistanceScale = 1.0 / max(zDepth * 0.3, 1.0);
    vec3 noise = fractalNoise(model_Coords.xyz * noiseDistanceScale, u_time, luminance);
    color.rgb = mix(color.rgb, noise, adaptiveIntensity * 0.2 * noiseDistanceScale);
